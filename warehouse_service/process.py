...
def save_message(cursor, message):
    ...
    event_id = create_event(cursor, message.lims_id, message.uuid, event_type_id, message.timestamp,
                            message.user_identifier)

    for role in message.roles:
        subject_type_id = subject_type_map[role.subject_type]
        subject_id = find_or_create_subject(cursor, role.subject_uuid, role.subject_friendly_name,
                                            subject_type_id)
